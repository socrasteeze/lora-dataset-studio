...
- **🖥️ Rent another machine’s GPU without moving your datasets** — run the app on two boxes (e.g. a 5090 desktop and a laptop over Tailscale): set one as **Primary** (owns the datasets) and join the other as a **compute peer**. Generate with a **Run on** picker aimed at either machine; results always land back on the Primary. What a peer runs today is **generation, and only generation** — training, captioning and scoring still run on the Primary whatever the picker is set to. The peer must be awake, the models for a job must exist on the machine that runs it, and joining a Primary means trusting it to run code on the peer — see **Settings → Devices**.
...
- **🚀 Launch all** — runs the entire chain end to end overnight and leaves a **morning report**. Several banks to clean? **Add to queue** lines them up, or **⏳ Queue all** takes every bank that still has undecided images in one gesture. They run **one at a time**, each waiting for the GPU rather than failing when another bank or a training run has it — never in parallel. And a run that could not take the GPU says so **on the bank card** the next morning (*"2 passes skipped"*), instead of looking exactly like a clean night.
...
**4. Promote** — **Promote all kept** into a target dataset (the counter is **per-target**, so "nothing to promote" means those images are undecided, not kept), and resolve **duplicate groups** as you go.
...
To **rent another PC’s GPU** while keeping datasets on one machine, use **Settings → Devices**: set the dataset machine to **Primary**, install the app on the second box as a **Peer**, and join with a one-time token over Tailscale. **Generation** can then pick either device; the image always writes back to the Primary. What this does *not* do yet: training, captioning and scoring ignore the picker and run on the Primary — the peer-side plumbing for them exists at the API level but no screen drives it, and a peer-run training returns loose checkpoint files rather than a run you can see on the Training page. The peer must be online, each job’s models must exist on the box that runs it, and a peer executes work its Primary sends — join only a Primary you control.
