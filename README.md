...
| **Caption tools and recovery** | Find/replace, tag frequencies, expanded editing, targeted re-captioning, stoppable batches and reload-proof recovery |
| **🧪 Caption Lab** | Bench up to four caption configurations — engine, vision model, vocabulary register, length — on **one** image and read them side by side against the caption already stored, before spending a pass on thousands. Nothing is written until you keep a result. On **both surfaces**: a dataset's Captions section (and any image's caption editor), and an image bank's 🏷️ Caption window |
...
| **🚩 Find & 🧽 clean watermarks** | Flags overlaid logos/URLs with a box, then removes them in two manual passes — a model-free crop, or a repaint into a *separate* file: LaMa touches only the marked zones, Klein erases them and re-renders the whole photo, which also reaches tiled or on-subject marks the scan missed |
| **🔤 Find text** | Reads burned-in text — speech bubbles, subtitles, captions, sound effects — and turns each block into a repaint zone the existing 🧽 clean funnel consumes, so one ↩ Undo covers both. CPU-only, offline, on the small OCR the video bank already ships. Try it on a sample first and tune the **Sensitivity** before committing a whole bank. Positions only: no transcript is ever stored |
...
| **🔤 Find text** | The same funnel, fed by OCR instead of a detector: burned-in dialogue, subtitles and captions become repaint zones, on banks and datasets alike. Needs the **Burned-in text** extra from Setup (RapidOCR, CPU-only, weights inside the wheel). Very stylised sound-effect lettering can still escape it — the mask editor covers those. Launching it keeps its window open: the flagged pages fill in live below the dials with their zones drawn on, so a sample run is judged where it was launched |
| **What to clean** | The repaint level takes a target — Both, 🔤 text-flagged pages, or 🚩 watermark-flagged ones — beside the LaMa/Klein toggle, on banks and datasets alike. The split is **by page**: a page carrying both counts as text and is repainted whole, so one page is never split between two runs. Picking a family with nothing in it refuses by naming the other one |
