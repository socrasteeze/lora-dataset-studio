...
Public repo â€” everything here is visible; keep it free of personal data.
...
- Commits are authored as `lora-dataset-studio <noreply@lora-dataset-studio.dev>`.
  ...
  will happily author commits as someone else â€” this has happened. Set it once,
  ...
  merge parents), never with `rebase` â€” a rebase across a sync would rewrite the
...
- No real names, usernames, machine paths (`C:\Users\...`), IPs or tokens in
  ...
  paste-safe (path redaction helpers exist â€” reuse them).
...
- `backend/tests/test_no_personal_data.py` enforces the two rules above.
  ...
  or `LDS_PRIVACY_NAMES`) â€” writing them here to forbid them would publish them;
  with no list that half SKIPS and says so.

## Tests â€” targeted while you work, full and green before you push
...
**Every `python` below means `.venv`'s interpreter** â€” `.venv/Scripts/python.exe`
on Windows, `.venv/bin/python` on POSIX â€” never the machine default. See "Run the
suite through `.venv`" at the end of this section for why, and for the one command
that provisions it.

- **While coding** â€” only what can tell you something about what you just
  changed: `python -m pytest backend/tests/test_foo.py -q` (test files
  are named after the module or domain they cover: `app/services/foo.py` â†’
...
- **Before a commit** â€” the above, plus the tests no filename can lead you to:
...
- **Before the push that LANDS the wave** â€” both suites, whole and green, on
  ...
  repo root and `npm run lint` from `frontend/` â€” CI's Lint job runs OUTSIDE the
  ...
  interpreter, and neither is right here â€” see the `.venv` rule below, which this
...
- **Before an intermediate push on a branch** â€” the
  ...
  `git diff --name-only <base>...HEAD | grep -c '^backend/'` â€” zero means the
  ...
  touched no Python at all. At the flake rate above â€” one red per five runs,
  never reproducible â€” the superfluous runs did not merely cost six minutes
...
- **Before a release** â€” nothing by hand: `release.yml` reruns both suites
  unconditionally. Do not tag until that workflow is green.

Parallel runs are safe here â€” xdist workers are separate processes, the app uses
...
is evidence about CI â€” and a system interpreter drifts the moment anything else
...
that `.venv` has THAT one â€” after any sync touching `requirements*.txt`, re-run
...
user actually gets it. Provision it once â€” **both** lines:
...
`torch` â€” the same under-guard `requirements-dev.txt`'s own header describes for
...
same tree runs **7 976 passed / 10 skipped / 0 failed** â€” the full 7 986 collected,
...
Torch in `.venv` is where the app puts it anyway â€” `setup_installer.py`'s
...
Call that `python.exe` **directly** â€” never `activate.bat`. A venv carries
...
They share features â€” the face pass, quality/scoring passes, watermark detect
...
surfaces carry it â€” at FULL parity: the dials, the launch window, the
counts, not just the pass underneath** (maintainer's standing rule,
2026-08-27: "full parity, always". The ðŸ”¤ Find text sample dial shipped
...
this takes: *two surfaces, two mechanics, ONE contract* â€” the plumbing may
...
exact same symptom on the dataset side â€” full-body and bust shots that never
...
- Port it. A difference between the surfaces â€” even a documented one â€” now
...
The machine you build on already has the dependency. The new user's does not â€”
so a feature can be finished, tested and green, and still land as a âœ— on their
...
- Make the capability probe import **everything** the feature imports at load
  time, not just its headline module. A probe that under-imports reports âœ“ while
  the feature dies on the first call â€” GitHub #24, where a masks install said
...
builds, always** â€” they are small, reliable and cross-OS (`_TORCH_CPU_INDEX`,
...
does NOT owe you a CUDA install action â€” it owes you a graceful CPU default, a
...
they cannot read your working copy â€” so work that lives only on your disk reads
...
- **Anything non-trivial goes on a branch, and that branch is pushed** before it
  lands â€” even when no PR is opened. Pushing IS the announcement. Name it for
...
- **`main` stays releasable.** A branch may be red while it cooks; `main` may
  ...
  `main`. An intermediate push on a branch is the "cooking" case â€” it owes the
...
- **Delete the branch once it lands, without being asked.** A stale remote
  branch claims work is in progress when it is finished â€” the same lie,
  ...
  stale â€” leave it and say what it holds, even when its feature reached `main`
  by another route.
- **Pushing to `origin` no longer waits for permission â€” branch or `main`.**
...
- **`upstream` stays read-only, always.** Never push to it, never open a PR
  ...
  a separate, deliberate act â€” see "Sending a PR UPSTREAM" below and
  `docs/UPSTREAM_SYNC.md`.

## Shipping checklist â€” the tail of EVERY user-visible wave
...
1. **Fork gates stay in the before-push run.** The full frontend suite includes
   ...
   cannot (three workspace-crashing `ReferenceError`s shipped this way â€” see
   FORK_NOTES merge diagnostic 6).
2. **Source-only commits** â€” dist rebuild is its own `build(frontend):` commit
   ...
   **This is a rule about commit GRANULARITY, not about whether dist ships.**
   ...
   `frontend/src` reaches people with its rebuilt bundle â€” "commit the
   regenerated `frontend/dist/` in the same PR" â€” because the repo ships
   ...
   that misreading â€” caught only by reading their PR template afterwards.
   **For an upstream PR, their `CONTRIBUTING.md` and
   `.github/PULL_REQUEST_TEMPLATE.md` are the authority, not this file** â€” read
   both before building the branch (see "Sending a PR UPSTREAM" below).
3. **ðŸŽ What's new**: one benefit-first entry per user-visible change
   (`frontend/src/whatsNew.js`) â€” release notes are built from it.
...
5. **Docs & README**: settings-reference on setting changes; README at every
   release â€” fix anything no longer true, and only new capabilities earn a line.
   After an **upstream merge**, also update `FORK_NOTES.md` if a divergence
   changed, and **rebuild `frontend/dist`** â€” Flask serves dist; taking
...
7. **Never rename catalog labels, config keys or What's-new ids** without an
   alias path â€” several of them are stored in user databases and localStorage.
8. **Windows scripts (`.ps1`/`.bat`/`.cmd`) and `requirements*.txt` stay
   ...
   ANSI codepage, not UTF-8 â€” an em-dash decodes into a curly quote there,
...
10. **New dependency, model or capability? Wire the installer** â€” an action in
...
files involved. Releases are cut on validated waves only, never per commit â€”
...
**The procedure lives in [`docs/UPSTREAM_SYNC.md`](docs/UPSTREAM_SYNC.md)** â€”
...
`FORK_NOTES.md` â€” especially **Divergence 1 (local-only generation)** and the
**Merge diagnostics** section (read it *before* touching a single conflict â€”
...
come â€” see Divergence 3 in `FORK_NOTES.md` for what it cost.
...
- **Branch off `upstream/main`, never off this fork's `main`.** A branch cut from
  `main` carries the entire divergence into a public PR â€” the cloud-engine
...
- **Their `CONTRIBUTING.md` and `.github/PULL_REQUEST_TEMPLATE.md` are the
  ...
  upstream â€” as of 2026-08-22 they DO have one (`ruff check .` at the root and
...
- **Run the FULL backend suite against the branch**, not just the file touched.
  ...
  `git status` before believing the change caused it** â€” then say so in the PR,
...
- **Author as the repo owner's GitHub identity**, since the PR comes from their
  ...
  time â€” do NOT write the address into this file or any tracked file, because
  ...
  (`git -c user.name=â€¦ -c user.email=â€¦ commit`). **Never `git config` it:**
