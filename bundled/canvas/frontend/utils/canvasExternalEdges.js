/* 🔌 EXTERNAL-LORA PROVENANCE — which board images an external board node
...
import { edgePath } from '@lds/plugin-sdk/canvas';
import { BOARD_NODE_TYPES, normalizeLoraName, nodeKey } from '../components/canvas/boardNodes/registry.js';
...
export function externalEdgesFor(imageNodes, lanes, extNodes, boxByKey) {
  ...
  for (const n of (imageNodes || [])) {
    ...
    for (const m of members) {
      ...
      const x1 = anchorX(box, BOARD_NODE_TYPES['external-lora']?.edge?.side);
