...
function warnUnknown(type) {
  ...
  console.warn('unknown board node type', type);
}

const idOf = (node) => node?.filename ?? node?.id;

function BoardNodeCard({ typeDef, node, store, boardScale, onGeometry }) {
...
function BoardAddFlow({ typeDef, store }) {
...
/* Generic renderer for every board-space board-node type: the absolute-
   ...
   `boardNodes/registry.js`; this component owns drag, geometry reporting
   and persistence wiring against whatever store the caller passes it. */
export default function BoardNodeLayer({ types, stores, boardScale = 1, onGeometry }) {
  return (
    <>
        ...
        return (
          <div key={type}>
            {(store.nodes || []).map((node) => (
              <BoardNodeCard key={idOf(node)} typeDef={typeDef} node={node} store={store}
                boardScale={boardScale} onGeometry={onGeometry} />
            ))}
            {store.extra?.pickerOpen && <BoardAddFlow typeDef={typeDef} store={store} />}
          </div>
        );
      })}
    </>
