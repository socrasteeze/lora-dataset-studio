...
test('the provenance layer is under everything and never takes a click', () => {
  const canvas = readSource('../bundled/canvas/frontend/components/canvas/LineageCanvas.jsx');
...
test('the badge is rendered from the note, and only when there IS one', () => {
  const node = readSource('../bundled/canvas/frontend/components/canvas/CanvasImageNode.jsx');
