...
} from '../frontend/utils/canvasImageNodes.js';
...
/* A rejected board write must not be silently ignored.

   `save_canvas_image_nodes` returns 200 with `saved: 0` when it rejects a row
   (invalid geometry or an image from another dataset). Ignoring that response
   displayed an unsaved image that disappeared on reload without explanation. */
