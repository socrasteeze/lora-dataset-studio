...
import { BOARD_NODE_TYPES, nodeKey, knownType } from '../frontend/components/canvas/boardNodes/registry.js';

test('external-lora is a registered type with the full contract', () => {
  const t = BOARD_NODE_TYPES['external-lora'];
...
test('external-lora payload returns only the checked nodes, in the shape genSettings expects', () => {
  ...
  const payload = BOARD_NODE_TYPES['external-lora'].payload(nodes, checked);
