...
const page = fs.readFileSync(new URL('../frontend/pages/CanvasPage.jsx', import.meta.url), 'utf8');
const filter = fs.readFileSync(new URL('../frontend/components/canvas/CanvasDatasetFilter.jsx', import.meta.url), 'utf8');
...
test('a control is written once and PLACED, never rendered twice', () => {
  const hook = fs.readFileSync(new URL('../../../frontend/src/hooks/useMediaQuery.js', import.meta.url), 'utf8');
...
test('the optional resource monitor is contributed independently on the board', () => {
  const mount = fs.readFileSync(new URL('../frontend/components/canvas/CanvasSystemStats.jsx', import.meta.url), 'utf8');
  assert.match(mount, /PluginSlot slot="resource_monitor.readout" surface="canvas"/);
  assert.doesNotMatch(mount, /SystemStatsReadout|MACHINE_LOAD_PREF_KEY/);
});
...
test('the filter chips drop their words on a phone, never their counts', () => {
  const menu = fs.readFileSync(new URL('../frontend/components/canvas/CanvasFilterMenu.jsx', import.meta.url), 'utf8');
