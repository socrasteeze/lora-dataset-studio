...
from lds_video import video_safe_zone_geometry as geometry
...
def text_engine_reason():
    ...
    """
    from lds_sdk.video_host.capabilities import probe_video_text
...
def _write_frames(src_path, times, dest_dir, stem, long_side=None):
    ...
    from lds_video.video_clip_search import _write_frames as write
...
def _clip_frame_times(clip):
    ...
    EDGE_MARGIN_S moves."""
    from lds_video.video_clip_search import _clip_frame_times as times
    return times(clip)
...
def _extract_chunk(bank, chunk, relpaths, scratch):
    ...
    """
    from lds_video.video_bank_service import _abs_source_path
    extracted = {}
    frames = []
    for clip in chunk:
