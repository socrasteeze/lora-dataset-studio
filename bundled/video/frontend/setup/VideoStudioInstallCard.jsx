import { admittedVideoPlan, videoBatchProgress } from '../lib/videoPreparation.js'
import { useEffect, useRef, useState } from 'react'
import { apiFetch, postJson } from '@lds/plugin-sdk'
import { useToast } from '@lds/plugin-sdk'
import { INSTALL_ALL_ACTION_LABELS, videoStudioInstallPlan } from '../lib/videoSetup.js'
...
export default function VideoStudioInstallCard({ caps, onDone }) {
  ...
  const poll = (actions) => {
    apiFetch(`/api/setup/install-all/status?actions=${actions.join(',')}`).then((r) => {
      ...
      setStatuses(st)
      let progress
      try { progress = videoBatchProgress(actions, st) } catch (e) {
      ...
      if (!mounted.current || ++pollFailures.current < 3) return
      clearInterval(timer.current)
      setPhase('idle')
      toast.error(e.message || 'Could not read installation progress. Re-check before retrying.')
    })
  }
  ...
  const start = async () => {
    ...
    setPhase('running')
    try {
      const response = await postJson('/api/plugins/video/preparation', { actions: plan })
      if (!mounted.current) return
      const admitted = admittedVideoPlan(plan, response)
      const initial = videoBatchProgress(admitted, response.statuses)
      setTracked(admitted)
      setStatuses(response.statuses)
      if (initial.terminal) { setPhase('done'); onDone?.(); return }
      timer.current = setInterval(() => poll(admitted), POLL_MS)
      poll(admitted)
    } catch (e) {
      if (!mounted.current) return
      clearInterval(timer.current)
