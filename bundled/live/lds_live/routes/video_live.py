...
from lds_sdk.video_host import netguard
...
def live_start():
    ...
    if not isinstance(data, dict):
        return jsonify(error='Expected Live channel settings.'), 400
    lora = data.get('lora') or None
    if lora:
        from lds_sdk.video_host import studio as lts
        if lts.unsafe_lora_name(lora):
...
@bp.before_request
def require_live_product():
    from lds_sdk.lifecycle import is_available
    if not is_available('live'):
        return jsonify(ok=False, code='plugin_unavailable',
                       error='Live is disabled. Enable it in Plugins and restart LDS.'), 409

@bp.get('/render-options')
def live_render_options():
    from lds_live.setup import missing_weights
    from lds_sdk.local_render import comfyui_reachable
    profile = vts.profile()
    classes = vts.registered_classes()
    return jsonify({
        'missing_weights': missing_weights(), 'ready': comfyui_reachable() and vts.studio_ready(missing_weights()),
        'options_available': vts.option_availability(classes),
        'sage': vts.sage_available(classes), 'frame_choices': list(profile.get('frame_choices') or ()),
        'frame_default': vts.FRAMES_DEFAULT, 'fps': profile.get('fps'),
        'frames_min': vts.FRAMES_MIN, 'frames_max': vts.FRAMES_MAX,
        'megapixels': {'min': vts.MP_MIN, 'max': vts.MP_MAX, 'default': vts.MP_DEFAULT},
        'sparse_modes': list(vts.SPARSE_MODES), 'turbo_steps': vts.TURBO_STEPS,
        'default_steps': vts.DEFAULT_STEPS, 'eros_available': vts.eros_on_disk(),
    })

@bp.get('/loras')
def live_loras():
    return jsonify(deployed=vts.deployed_loras(), trained=vts.trained_loras())

@bp.post('/deploy')
def live_deploy():
    data = request.get_json(silent=True)
    if not isinstance(data, dict):
        return jsonify(error='Expected a checkpoint selection.'), 400
    try:
        return jsonify(ok=True, filename=vts.deploy_checkpoint(data.get('run_id'), data.get('filename')))
    except (ValueError, TypeError, OSError) as exc:
        from lds_sdk.media import redact_user_paths
        return jsonify(ok=False, error=redact_user_paths(str(exc))), 400

@bp.post('/lora/import')
def live_lora_import():
    upload = request.files.get('file')
    data = {} if upload is not None else request.get_json(silent=True)
    if not isinstance(data, dict):
        return jsonify(error='Expected a LoRA path or upload.'), 400
    try:
        result = vts.import_external_lora(src_path=data.get('path'), upload=upload,
