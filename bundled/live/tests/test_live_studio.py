...
def test_every_name_slot_becomes_the_subject():
    assert live.fill_scene('{NAME} meets {NAME2} and {NAME}', 'subject') == 'subject meets subject and subject'
...
def _params(**over):
    p = {'frames': 124, 'megapixels': 0.3, 'aspect': 'landscape', 'fps': 0, 'steps': 4,
         'turbo': True, 'scenes': live.DEFAULT_SCENES, 'subject': 'subject', 'seed': 10}
    p.update(over)
    return p
...
def test_the_producer_keeps_the_pipeline_full_and_no_more(app, stubbed):
    with app.app_context():
        ...
        assert 'subject' in wf['104']['inputs']['prompt'] and 'first_frame' not in wf['104']['inputs']
...
def test_stop_leaves_the_clip_on_the_card_alone_and_discards_it_when_it_lands(app, stubbed, monkeypatch, tmp_path):
    ...
    with app.app_context():
        ...
        monkeypatch.setattr(lts, 'comfy_output_dir', lambda: str(out))
