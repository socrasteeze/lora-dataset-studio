...
} from '../frontend/studio/live/liveStudioApi.js';
...
test('the start payload says auto as 0, leaves empty options out, carries the LoRA when picked', () => {
  const base = { scenes: 's', subject: 'subject', megapixels: '0.3', aspect: 'square', frames: '124',
    fps: 'auto', turbo: true, steps: 4 };
  const p = buildStartPayload(base);
  assert.deepEqual(p, { scenes: 's', subject: 'subject', megapixels: 0.3, aspect: 'square',
