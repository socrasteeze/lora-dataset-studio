...
import {
  ...
  weightHint,
} from '../frontend/lib/loraMerge.js';
...
const tool = readFileSync(
  fileURLToPath(new URL('../frontend/panels/LoraMergeTool.jsx', import.meta.url)), 'utf8');
...
const panel = readFileSync(
  fileURLToPath(new URL('../frontend/panels/TrainingMergeTool.jsx', import.meta.url)), 'utf8');
...
test('the merge disclosure is controlled, so a remount cannot close it', () => {
  ...
  // (the core's `training.tool` slot), and every swap of that portal's host
  // unmounts it — which is how rotating a phone closed the tool on top of
  // emptying it. The panel reads the persisted value back on every mount.
