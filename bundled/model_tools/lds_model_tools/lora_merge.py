...
and LDS installs and runs without it). This file is the shared, stdlib-only
header and planning contract. The app imports it directly; the separately
packaged lora_merge_worker.py imports the same helper beside its script and
runs tensor operations in the configured interpreter.

torch is the ONLY thing the worker needs. Reading and writing safetensors is done
by the worker against this contract’s 8-byte length + JSON header — see its
``Reader`` for why ordinary file I/O fixes a measured memory-map crash.
