...
not (it is gigabytes, and LDS installs and runs without it). So ``lora_merge_worker`` is
...
from lds_sdk import models, workers

queue_manager = ModelToolState()

from . import fp8_quantize, lora_merge, runtime

logger = logging.getLogger(__name__)

_WORKER = os.path.join(os.path.dirname(__file__), 'lora_merge_worker.py')
...
def resolve_ref(ref, folder_types):
    ...
    for folder_type in folder_types:
        try:
            hit = models.resolve_model_file(folder_type, ref)
...
def _base_family(path, header):
    ...
    try:
        ...
        return _BASE_ARCH_TO_FAMILY.get(models.detect_safetensors_arch(keys))
...
def plan(base, loras, *, destination=None, destination_dir=None,
    ...
    integrity = models.validate_model_file(base)
    ...
    # depending on it.
    report = models.quantization_report(base)
    if report.get('quantized'):
        signals = ', '.join(report.get('signals') or []) or 'quantized dtypes'
        if report.get('form') == models.FORM_STRUCTURED:
    ...
    for index, item in enumerate(loras):
        ...
        detected = models.detect_lora_arch(path)
        if family and detected and models.lora_arch_conflicts(detected, family):
            raise MergeJobError(
...
def worker_command(python, spec_path, *, budget_seconds=DEFAULT_BUDGET_SECONDS) -> list:
    ...
    return workers.isolated_worker_argv(python, os.path.abspath(_WORKER),
...
def run_worker(info, *, progress=None, cancelled=None,
    ...
    try:
        with runtime.use_worker(info['python']) as selected:
            return _run_worker({**info, 'python': selected}, progress=progress,
                               cancelled=cancelled, budget_seconds=budget_seconds)
    except RuntimeError as e:
        raise MergeJobError(str(e)) from e


def _run_worker(info, *, progress=None, cancelled=None,
                budget_seconds=DEFAULT_BUDGET_SECONDS) -> dict:
    """Run the merge in the configured interpreter and stream its progress back.

    The child imports the same stdlib header contract, so there is one merge and one
    ...
    try:
        try:
            proc = subprocess.Popen(
                ...
                env=workers.isolated_worker_env(info['python']),
                cwd=os.path.dirname(os.path.abspath(_WORKER)),
                creationflags=getattr(subprocess, 'CREATE_NO_WINDOW', 0))
        except OSError as e:
            raise MergeJobError(
                f'the Python that would do the merge could not be started: '
                f'{info["python"]} ({e}). Open Plugins > Model tools > Settings '
                'to check the CPU engine or the Advanced Python override.') from e
