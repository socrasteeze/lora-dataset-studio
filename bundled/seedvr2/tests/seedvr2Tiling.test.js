...
import { ceilingLine, laneForTarget, tilingStatus, TTP_PACK, TTP_URL } from '../frontend/lib/seedvr2Tiling.js'

const caps = (comfyui) => ({ comfyui: { reachable: true, ...comfyui } })

test('an unavailable node list asks to re-check without inventing missing nodes', () => {
  for (const missing of [null, undefined, { ok: false }, []]) {
    const s = tilingStatus(caps({ seedvr2_tiling_ready: false,
      seedvr2_tiling_nodes_missing: missing }))
    assert.equal(s.state, 'unknown')
    assert.match(s.text, /Re-check/)
    assert.doesNotMatch(s.text, /[Ii]nstall|[Uu]pdate|[Rr]estart|are missing/)
  }
})

test('missing standard nodes require a ComfyUI update, not another TTP install', () => {
  const s = tilingStatus(caps({ seedvr2_tiling_ready: false,
    seedvr2_tiling_nodes_missing: ['ImageCrop', 'ImageBatch'] }))
  assert.equal(s.state, 'restart')
  assert.match(s.text, /Update ComfyUI and restart/)
  assert.match(s.text, /ImageCrop/)
  assert.match(s.text, /ImageBatch/)
  assert.match(s.text, /TTP pack alone does not add them/)
})
