...
export function tilingStatus(caps) {
  ...
  const missing = c.seedvr2_tiling_nodes_missing
  if (!Array.isArray(missing) || !missing.length) {
    return { state: 'unknown',
      text: 'ComfyUI is reachable, but its node list could not be checked. '
        + 'Re-check in a moment to confirm whether tiling is available.' }
  }
  const standard = missing.filter(name => !name.startsWith('TTP_'))
  if (standard.length) {
    return { state: 'restart',
      text: `Update ComfyUI and restart it: its standard image/mask nodes `
        + `(${standard.join(', ')}) are missing. They are needed to preserve overlap `
        + 'at tile boundaries; installing the TTP pack alone does not add them.' }
  }
