...
from lds_sdk import models as comfy_model_paths
from lds_sdk import comfy as comfy_fs
from lds_sdk.comfy import queue as queue_manager
...
TILING_STANDARD_CLASSES = ('ImageCrop', 'ImageBatch', 'EmptyImage', 'ImageFromBatch',
                           'ImageCompositeMasked', 'SolidMask', 'FeatherMask', 'MaskComposite')
TTP_NODE_PACK = {
...
# Two 64px blend bands leave context around restored tile edges. This is a
# geometry safeguard, not a new preference or a guarantee of model quality.
TILE_MIN_OVERLAP = 128
...
def full_frame_ceiling_mp(vram_gb=_PROBE_VRAM):
    ...
    if vram_gb is _PROBE_VRAM:
        from lds_sdk import local_render as capabilities
...
def _ttp_axis(length, side):
    """The supported TTP tiler's count and step, including exact multiples.

    TTP chooses ceil(length / side), then shares the excess between neighbours.
    It does not accept an overlap-rate input. Exact multiples have no overlap.
    The frozen upstream CPU outputs in the geometry tests cover this contract.
    """
    count = max(1, -(-length // side))
    overlap = (count * side - length) // (count - 1) if count > 1 else 0
    return count, side - overlap if count > 1 else 0


def _ttp_positions(plan):
    """Row-major crop boxes, including TTP's correction of the final edge."""
    width, height = int(plan['output_width']), int(plan['output_height'])
    tw, th = int(plan['tile_width']), int(plan['tile_height'])
    columns, dx = _ttp_axis(width, tw)
    rows, dy = _ttp_axis(height, th)
    return [(min(x * dx, max(0, width - tw)),
             min(y * dy, max(0, height - th)),
             min(x * dx + tw, width), min(y * dy + th, height))
            for y in range(rows) for x in range(columns)]


def _overlap_plan(plan, padding=64):
    """Keep safe TTP geometry; otherwise specify bounded overlapping crops.

    TTP's minimum covering grid can have no context, or an odd overlap whose
    centred PIL crops differ by one pixel. Neither is fixed by more padding.
    Explicit crops keep the requested tile size and frame unchanged. Only the
    number of tiles can grow; the caller must use this same plan for its title.
    """
    if 'positions' in plan:
        return plan
    columns = plan['columns']
    positions = _ttp_positions(plan)
    minimum = min(TILE_MIN_OVERLAP, plan['tile_width'] // 2, plan['tile_height'] // 2)
    overlaps = [overlap for i, (left, top, _, _) in enumerate(positions)
                for overlap in ((positions[i - 1][2] - left,) if i % columns else ())
                + ((positions[i - columns][3] - top,) if i >= columns else ())]
    if all(o >= minimum and (o - min(o, max(0, int(padding)))) % 2 == 0 for o in overlaps):
        return plan

    def starts(length, side):
        if length <= side:
            return [0]
        stride = side - minimum
        count = 1 + (length - side + stride - 1) // stride
        return [i * (length - side) // (count - 1) for i in range(count)]

    width, height = plan['output_width'], plan['output_height']
    tw, th = min(plan['tile_width'], width), min(plan['tile_height'], height)
    xs, ys = starts(width, tw), starts(height, th)
    return {**plan, 'columns': len(xs), 'rows': len(ys), 'tiles': len(xs) * len(ys),
            'positions': [(x, y, x + tw, y + th) for y in ys for x in xs]}
...
def tile_plan(width, height, short_edge, tile_px=TILE_PX):
    ...
    columns, _ = _ttp_axis(out_w, side)
    rows, _ = _ttp_axis(out_h, side)
    if columns * rows <= 1:
        return None
    return _overlap_plan({'tile_width': side, 'tile_height': side,
...
def seedvr2_invalid_assets():
    ...
    from lds_sdk import models as model_integrity
    out = []
...
# at once. The base-engine probe fails open on transient discovery failures;
# optional tiling must have confirmed nodes before it can be advertised.
_NODES_OK_TTL_S = 300
_nodes_ok_until = 0.0
_ttp_ok_until = 0.0
...
def seedvr2_missing_nodes():
    ...
    if time.time() < _nodes_ok_until:
        return []
    from lds_sdk.local_render import fetch_object_info_classes
...
def ttp_missing_nodes():
    ...
    Includes the standard Comfy nodes needed for unsafe TTP geometry. Success
    is cached, misses never. None means discovery was unavailable, not that
    nodes are missing. The optional full-frame lane remains independent."""
    ...
    if available is None:
        return None
    out = sorted(c for c in TTP_NODE_CLASSES + TILING_STANDARD_CLASSES if c not in available)
    if not out:
...
def tiling_available(comfy_ok=True):
    ...
    probe cannot fail open into a promise), TTP and the standard fallback nodes."""
    if not comfy_ok:
        return False
    missing = ttp_missing_nodes()
...
def _cut_overlapping_tiles(graph, plan):
    """Build a balanced batch tree, rather than retaining 2+3+...+N tiles.

    Inference still has one tile's activations. Comfy can cache crop/batch and
    full-frame assembly outputs: more overlap uses more RAM and more passes,
    not a promise of constant total memory. No shared node pack is changed.
    """
    pending = []
    for index, (left, top, right, bottom) in enumerate(plan['positions']):
        name = f'cut_{index}'
        graph[name] = {'class_type': 'ImageCrop',
                       'inputs': {'image': ['4', 0], 'width': right-left, 'height': bottom-top,
                                  'x': left, 'y': top}}
    ...
    while len(pending) > 1:
        ...
        for i in range(0, len(pending), 2):
            if i + 1 == len(pending):
                next_level.append(pending[i])
                continue
            name = '5' if len(pending) == 2 else f'cut_batch_{level}_{i // 2}'
            graph[name] = {'class_type': 'ImageBatch',
                           'inputs': {'image1': pending[i], 'image2': pending[i+1]}}
            next_level.append([name, 0])
        pending = next_level
        level += 1
    graph['5']['_meta'] = {'title': f"Cut into {plan['tiles']} tiles with overlap"}


def _assemble_overlapping_tiles(graph, plan, padding):
    """Place explicit crops on a full canvas with centred, bounded feathering.

    Finish each row before blending rows: multiplying both axes' masks while
    painting tiles directly can retain the preceding tile's bad corner. Row
    buffers also avoid caching one full-height canvas for every tile.
    """
    columns = int(plan['columns'])
    width, height = int(plan['output_width']), int(plan['output_height'])
    graph['7'] = {'class_type': 'EmptyImage',
                  'inputs': {'width': width, 'height': height, 'batch_size': 1, 'color': 0},
                  '_meta': {'title': 'Full target canvas'}}
    positions = plan['positions']
    def paste(prefix, destination, source, left, top, size, overlap_x=0, overlap_y=0):
        inputs = {'destination': destination, 'source': source,
                  'x': left, 'y': top, 'resize_source': False}
        if overlap_x or overlap_y:
            band_x, band_y = (min(max(0, int(padding)), overlap)
                              for overlap in (overlap_x, overlap_y))
            offset_x, offset_y = (overlap_x - band_x) // 2, (overlap_y - band_y) // 2
            mask, feather = f'{prefix}_mask', f'{prefix}_feather'
            graph[mask] = {'class_type': 'SolidMask', 'inputs': {'value': 1.0, **size}}
            graph[feather] = {'class_type': 'FeatherMask',
                             'inputs': {'mask': [mask, 0], 'left': band_x, 'top': band_y,
                                        'right': 0, 'bottom': 0}}
            inputs['mask'] = [feather, 0]
            if offset_x or offset_y:
                blank, shifted = f'{prefix}_blank', f'{prefix}_shifted'
                graph[blank] = {'class_type': 'SolidMask', 'inputs': {'value': 0.0, **size}}
                graph[shifted] = {'class_type': 'MaskComposite',
                                  'inputs': {'destination': [blank, 0], 'source': [feather, 0],
                                             'x': offset_x, 'y': offset_y, 'operation': 'add'}}
                inputs['mask'] = [shifted, 0]
        name = f'{prefix}_paste'
        graph[name] = {'class_type': 'ImageCompositeMasked', 'inputs': inputs}
        return [name, 0]

    last = ['7', 0]
    for row in range(plan['rows']):
        first = row * columns
        top, bottom = positions[first][1], positions[first][3]
        size = {'width': width, 'height': bottom-top}
        row_prefix = f'assemble_row_{row}'
        canvas = f'{row_prefix}_canvas'
        graph[canvas] = {'class_type': 'EmptyImage',
                         'inputs': {**size, 'batch_size': 1, 'color': 0}}
        row_output = [canvas, 0]
        for index in range(first, first + columns):
            left, _, right, _ = positions[index]
            prefix, tile = f'assemble_{index}', f'assemble_{index}_tile'
            graph[tile] = {'class_type': 'ImageFromBatch',
                           'inputs': {'image': ['6', 0], 'batch_index': index, 'length': 1}}
            overlap = positions[index-1][2] - left if index > first else 0
            row_output = paste(prefix, row_output, [tile, 0], left, 0,
                               {'width': right-left, 'height': bottom-top}, overlap_x=overlap)
            graph[row_output[0]]['_meta'] = {'title': f'Place tile {index + 1} of {len(positions)}'}
        overlap = positions[first-columns][3] - top if row else 0
        last = paste(row_prefix, last, row_output, 0, top, size, overlap_y=overlap)
        graph[last[0]]['_meta'] = {'title': f"Blend row {row + 1} of {plan['rows']}"}
    graph['8']['inputs']['images'] = last
...
def build_tiled_workflow(source_image, *, dit, vae, seed, plan,
    ...
    MIT instead of GPL-3.0, standard Comfy nodes for unsafe grids, and geometry
    that a test can check without a ComfyUI at all.

    The upscaler runs on a tile BATCH from TTP or explicit standard-node crops.
    A single SeedVR2 call covers the batch and its inference window never holds
    ...
    plan = _overlap_plan(plan, padding)
    resolution = min(int(resolution), plan['tile_width'], plan['tile_height'],
    ...
    if 'positions' in plan:
        _cut_overlapping_tiles(graph, plan)
        _assemble_overlapping_tiles(graph, plan, padding)
    return graph
...
def _source_size(path):
    ...
    try:
        ...
        from lds_sdk import local_render as image_encoding
...
def choose_lane(width, height, *, short_edge, tiling_ok, ceiling_mp=None,
    ...
    plan = (tile_plan(width, height, short_edge, tile_px)
    ...
    if plan:
        return {'lane': 'tiled', 'plan': plan, 'output_mp': round(out_mp, 1),
                ...
                'notice': (f"Tiling this one: {plan['columns']}x{plan['rows']} tiles "
                           f"assembled into {round(out_mp, 1)} MP; "
                           f"SeedVR2 processes one tile at a time.")}
