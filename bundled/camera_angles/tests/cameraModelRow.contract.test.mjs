...
// The picker lives in the bundled camera-angles plugin now.
...
test('the slot the row lists from is registered by the plugin, as a diffusion-models slot', () => {
  const py = fs.readFileSync(
    path.join(process.cwd(), '..', 'bundled', 'camera_angles', 'lds_camera_angles', '__init__.py'),
    'utf8');
  assert.match(py, /register_model_slot\('camera_unet', folder_type='diffusion_models'/,
    'the plugin must register camera_unet or the combobox silently degrades to free text');
});
