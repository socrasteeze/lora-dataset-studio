...
from lds_sdk.comfy import load_workflow as load_workflow_local, queue as queue_manager
from lds_sdk.models import (resolve_lora_path as _lora_abs,
                            normalize_ref as normalize_rel_model_name,
                            resolve_ref as resolve_model_ref)

logger = logging.getLogger(__name__)

# The graph ships with the plugin, next to its package.
WORKFLOW_CAMERA_PATH = Path(__file__).resolve().parents[1] / 'workflows' / 'qwen_camera_angles.json'
...
def _spec(action):
    ...
    return setup_installer.model_download_spec(action)
