            ...
            try:
                ...
                for (row_id, _path), answer, error in source:
                    ...
                    # the dataset detector), never falsely mark everything clean.
                        ...
                        # would never be repainted). READ off the live row,
                        # WRITE through `pending`: the staged-write rule is
                        # about what this loop writes, not what it looks at.
                        ...
                        if bbox:
                            ...
                            if text_zones:
                                ...
                                merged, _ = text_mask_regions(
                                    ...
                                    + [[round(v, 4) for v in bbox]])
                                pending.setdefault(row_id, {}).update(
                                    watermark_regions=_json.dumps(merged))
                            detected += 1
                        else:
                            if text_zones:
                                pending.setdefault(row_id, {}).update(
                ...
                for rid in row_ids:
                    ...
                    if engine == 'klein':
                        ...
                        ok, err = watermark_klein.inpaint_watermark_klein(
                            ...
                            device_id=device_id, klein_model=klein_model)
                        generation_ok = _prepare_watermark_write(
                            row, src, expected_raw_fingerprint)
...
def _caption_job(bank_id, ids, force, vocabulary=None, length=None, device_id=None,
    ...
    def run(job):
        ...
        # What the LOCAL brick handled WITHOUT a caption. `_on_caption` only ever
        # hears about captions that landed, so a pass whose images were refused by
        # the GPU fence — 29 of 30, on the install that reported it — used to end
        # on "done — 1 captioned" and nothing else, which reads as "looked at them
        # all". These numbers are the missing half of that line. It stays empty on
        # the peer lane below: `_remote_caption` is fork-only and reports no
        # outcome breakdown, so those passes read exactly as they did before.
        left = {}
