...
def test_the_pass_reports_named_phases_then_a_per_image_count(app, tmp_path, monkeypatch):
    ...
    paths = [os.path.join(svc._dataset_dir(ds.id), f'k{i}.png') for i in range(3)]
...
def test_the_route_streams_the_count_into_a_joinable_job(app, client, tmp_path, monkeypatch):
    _use(monkeypatch, tmp_path, 'good.py', _GOOD)
    ds = _dataset(tmp_path, n=3)
    r = client.post(PREVIEW.format(ds.id), json={'limit': 6})
    assert r.status_code == 202
    # TESTING runs the job inline, so by now it is finished and published.
    body = client.get(PREVIEW.format(ds.id)).get_json()
...
def test_a_dead_subprocess_ends_as_a_message_not_an_endless_wait(app, client, tmp_path,
    ...
    client.post(PREVIEW.format(ds.id), json={})
    job = client.get(PREVIEW.format(ds.id)).get_json()['job']
...
def test_the_tool_being_absent_is_answered_immediately(app, client, tmp_path, monkeypatch):
    ...
    ds = _dataset(tmp_path, n=1)
    r = client.post(PREVIEW.format(ds.id), json={})
    assert r.status_code == 409
    assert r.get_json()['reason'] == 'face_scoring'
    assert fmp.get(ds.id) is None
...
def test_a_second_click_joins_the_running_pass_instead_of_starting_another(app, tmp_path):
    ...
    ds = _dataset(tmp_path, n=1)
    calls = []
    job, started = fmp.start(app, ds.id, lambda j: calls.append(j), total=1)
    ...
    live = {'phase': 'loading', 'done': 0, 'total': 1, 'error': None,
            ...
            '_fp': None}
    fmp._state[ds.id] = {'job': live, 'result': None}
    _, started2 = fmp.start(app, ds.id, lambda j: calls.append(j), total=1)
...
def test_a_preview_of_images_that_changed_is_flagged_stale(app, client, tmp_path, monkeypatch):
    ...
    ds = _dataset(tmp_path, n=2)
    client.post(PREVIEW.format(ds.id), json={})
    assert client.get(PREVIEW.format(ds.id)).get_json()['result']['stale'] is False

    fn = 'added.png'
    Image.new('RGB', (64, 64)).save(os.path.join(svc._dataset_dir(ds.id), fn))
    db.session.add(FaceDatasetImage(dataset_id=ds.id, status='keep', filename=fn))
    db.session.commit()
    assert client.get(PREVIEW.format(ds.id)).get_json()['result']['stale'] is True
