...
- Frontend: `VariationCatalog.jsx` (single Klein card), `EnginesSection.jsx`
  ...
  `face_single`/`face_multi` cards, any `CHATGPT_AUTH_OPTIONS`-style block and
  upstream's `ModelField` per-API-engine model card stay dropped, no
  Gemini/OpenAI secret fields), `settings/PromptPreview.jsx` (engine picker is
