...
| **Cloud / rented-GPU training** | Not available in this fork at all. Upstream rents a vast.ai GPU for a run; that lane is removed here, and the backend behind it is dormant and unreachable from the UI. **Settings → Devices** sends generation and Image Bank passes to another machine, never a training run | Train on this machine's GPU, or train elsewhere and import the `.safetensors` |
...
| **Mixed-version compute peers** | There is no version handshake between a Primary and its peers. A peer on older code is handled defensively (tolerant result parsing, a fallback key for vision answers) but nothing detects or reports the mismatch | Update the Primary and its peers together |
| **Embeddings caches written before 2026-08-01** | A ✨ Score or 👥 Group by person pass run on a peer before that date wrote a cache the local pass cannot read, because two arrays were dropped in transit. Those arrays were never uploaded, so the file cannot be repaired | Nothing to do — the next pass on that bank recomputes once, and every pass after it resumes normally |
| **Pass coverage for ✂ Find crops & variants** | It is bank-global, with no cheap per-image "still pending" answer, so it is never reported as done and always re-runs when selected. 🧹 Auto-reject is the same by choice (it is cheap and re-applies your current flags) | Untick either pass when you do not want it re-run |
