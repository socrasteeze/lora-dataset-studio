...
- **Ordinary fields wait for Save.** Change a field and a floating **Unsaved changes** bar appears with **Save** and **Discard**. Navigate away with changes pending and they're kept in the bar, not written.
...
- This holds when ComfyUI reaches the folder **through a link**, which is how most large collections are held together: a `models` folder pointing somewhere else, or a single family folder sent to another drive. The file is recognised where it already sits — the path you pinned and the path ComfyUI knows are compared as the same directory, not as two strings.
- A **full path outside every model folder** — Downloads, a Hugging Face cache, an unregistered drive — is **reported as such**, naming the folder to register. The app does not copy or link your weights anywhere on your behalf: move the file under `models/`, or add its folder to `extra_model_paths.yaml`. (Until 2026-09-16 such a pin was hardlinked into an `lds-pinned/` folder and shown as Found — free on one volume, but a real multi-GB copy across drives, which is exactly the setup that produced external pins. Any leftover `lds-pinned/` folder is safe to delete.)
- A reference that **cannot be resolved** — no such file, or a file outside every registered folder — is **refused rather than substituted**, with a badge here and the file named. It used to fall back to auto-detection and only show the badge: the run then went ahead on a *different* file from the one displayed, and the only symptom arrived with the images. **Clearing the field is the explicit way back to auto-detection.**
...
- **Civitai API key** → `CIVITAI_API_KEY` (secret). Optional. One key, two uses: without it Civitai scans return **SFW results only** (add it to reach adult content you're entitled to use), and it reads the prompts in the 🌐 Civitai browser. Free accounts have one: civitai.com → Account settings → API Keys.
