...
- **Only generation — and the bank's 🧽 Klein watermark inpaint — actually go to a peer or a backend.** The **Run on** picker appears on Generate and on the bank's watermark Level-3 card (Klein engine only; LaMa always runs here). Training, captioning, the queued bank passes (Score, faces, dedup, watermark detection) and every other GPU job run on the Primary regardless of what the picker says — the peer-side execution for them exists but nothing in the UI drives it, and a peer-run training leaves loose checkpoint files under `data/cluster_artifacts/` instead of a run on the Training page.
