...
const TOPICS = [
  ...
  setupStep('setup-krea-install', 'install', 'Install the Krea 2 Edit engine',
    ['krea', 'krea 2', 'krea 2 edit', 'install krea', 'node pack', 'comfyui-krea2edit',
     ...
     // DIVERGENCE 10 — hand-ported reword from upstream's help/topics/pages.js.
  ...
  // Hand-ported from upstream's split videoLane.js (Divergence 10).
  ...
  // ── Hand-ported from upstream's help/topics/videoLane.js + neuralRender.js
  //    (DIVERGENCE 10: the registry stays ONE file, so a new upstream topic is
  //    copied in rather than the split being adopted). Two keyword sets are
  //    TRIMMED on the way in, per Divergence 4: the rented-pod launch window's
  //    own topic is not ported at all, and the checkpoints/training topics drop
  //    the cloud verbs this fork does not render.
  ...
  action('video-dataset-captions', 'Captions of a video dataset',
    ['captions section', 'caption a clip', 'caption every clip', 'sidecar',
     'txt next to the mp4', 'find and replace', 'prefix', 'suffix', 'trigger word',
     'bulk captions', 'uncaptioned', 'no caption'],
    '/datasets', 'using-the-app', 'work-on-a-video-training-set'),
  action('video-dataset-references', 'Identity references of a video dataset',
    ...
    '/datasets', 'using-the-app', 'work-on-a-video-training-set'),
  // DIVERGENCE 4 — upstream's copy also answers to 'train in the cloud'.
  action('video-dataset-training', 'Training section of a video dataset',
    ['train section', 'launch training', 'steps', 'i2v', 'train this dataset',
     'train further'],
    '/datasets', 'using-the-app', 'work-on-a-video-training-set'),
  // DIVERGENCE 4 — upstream's copy also answers to 'cloud saves', 'run details',
  // 'delete a run' and the three 'continue from a step' phrasings. Those name
  // the ⓘ details card, the 🗑 run delete and ▶ Continue on a fresh pod, none of
  // which this fork renders; the ⬇ / 📦 / ⏏ / 🗑-a-save verbs it DOES render are
  ...
  action('video-dataset-checkpoints', 'Checkpoints & LoRAs of a video dataset',
    ['checkpoints section', 'video checkpoints', 'video lora files', 'saves',
     ...
     'delete a checkpoint', 'delete a save', 'trash a checkpoint',
     'local saves', 'no checkpoints yet', 'run graph', 'lineage', 'genealogy',
     'continued from', 'previews', 'samples', 'training samples', 'play a sample',
     'sample video', 'big previews'],
    '/datasets', 'using-the-app', 'work-on-a-video-training-set'),
  ...
  action('system-free-memory', '🧹 Free memory — unload what ComfyUI and the vision model keep in RAM',
    ['free memory', 'free ram', 'ram full', 'ram is full', 'memory full', 'out of memory',
     ...
     'free memory refused', 'try again when it finishes', 'press again', 'second press',
     'stop the render', 'interrupt the render', 'cancel does not stop the render',
     'render stuck', 'render will not stop'],
  ...
  // Upstream extended this topic with the lane grips this window shipped and
  // retitled it (D10's "silent reword": a count and an id-list diff both miss
  // it). Merged into the fork's own copy rather than added as a second topic.
