...
test('engine parts are recognised by what the graph grafts, candidates are not', () => {
  ...
  for (const f of ['h3/lds/lds174_video_Jessy___stills.safetensors',
...
test('a checkpoint filename reads as a name, a run and a step', () => {
  assert.equal(shortLoraName('h3/lds/lds174_video_Jessy___stills_000001000.safetensors'), 'Jessy — stills')
  ...
  assert.equal(checkpointStep('lds174_video_Jessy___stills_000001000.safetensors'), 1000)
  assert.equal(checkpointStep('lds174_video_Jessy___stills.safetensors'), null)
})
...
test('trained checkpoints group into one row per run, final first, newest run first', () => {
  const groups = groupTrained([
    ...
    { run_id: 174, dataset_id: 1, filename: 'lds174_video_Jessy___stills_000001000.safetensors', label: 'a', deployed_as: null },
    { run_id: 174, dataset_id: 1, filename: 'lds174_video_Jessy___stills.safetensors', label: 'b', deployed_as: 'h3/lds/lds174_video_Jessy___stills.safetensors' },
  ...
  assert.equal(groups[0].name, 'Jessy — stills')
  assert.deepEqual(groups[0].checkpoints.map((c) => [c.final, c.step]), [[true, null], [false, 1000]])
  assert.equal(groups[0].checkpoints[0].deployed_as, 'h3/lds/lds174_video_Jessy___stills.safetensors')
...
test('the folder list splits into candidates and engine parts, nothing dropped', () => {
  const { candidates, parts } = splitDeployed([
    { filename: 'h3/lds174_video_Jessy___stills.safetensors', label: 'j' },
