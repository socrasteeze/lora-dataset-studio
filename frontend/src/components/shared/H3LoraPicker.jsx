...
import { apiFetch, postForm, postJson } from '../../api/fetchClient.js';
...
export default function H3LoraPicker({ value, onChange, strength, onStrength, apiBase, lockKey = 'h3.lock.loraStrength' }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const d = await apiFetch(`${apiBase}/loras`);
  ...
  const runImport = async ({ path, file }) => {
    ...
    try {
      ...
      if (file) {
        ...
        fd.append('file', file);
        r = await postForm(`${apiBase}/lora/import`, fd);
      } else {
        r = await postJson(`${apiBase}/lora/import`, { path });
      }
      await load();
  ...
  const deployAndPick = async (ck, group) => {
    ...
    try {
      const r = await postJson(`${apiBase}/deploy`, { run_id: group.run_id, filename: ck.filename });
        ...
        <div className="flex flex-col gap-1.5">
          ...
          {!loading && !groups.length && !deployed.length && (
            <p className="text-xs text-content-subtle">
              No H3 LoRA yet — import a trained checkpoint, or drop a
